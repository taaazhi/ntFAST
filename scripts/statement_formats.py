...
FORMATS: Dict[str, StatementFormat] = {
    ...
    # Банк, для которого нет ни парсера, ни словаря заголовков. Ни одно из
    # этих названий не встречается в COLUMN_ALIASES, и словарный путь на нём
    # даёт 0 верных строк из 200 при 201 лишней: суммой становится остаток
    # по счёту, а первой «транзакцией» — строка периода. Отчёт при этом
    # выглядит успешным. Формат существует, чтобы этот провал был виден в
    # бенчмарке, а не обнаруживался у следователя.
    "unknown_bank": StatementFormat(
        key="unknown_bank",
        label="Незнакомый банк — нетиповые заголовки",
        language="ru",
        headers=["Күні/Дата", "Мәні", "Кому/от кого", "Кредит теңге",
                 "Дебет теңге", "Сальдо"],
        row_cells=_ru_cells,
        col_widths=[58, 78, 132, 68, 68, 72],
        preamble=[
            "«Тұран Банк» АҚ",
            "Шот бойынша көшірме / Выписка по счёту",
            "Кезең/Период: 05.01.2025 — 04.07.2025",
        ],
    ),
