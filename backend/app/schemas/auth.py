...
class UserResponse(UserBase):
    ...
    Адрес отдаётся как обычная строка, а не как `EmailStr`. Проверять его
    на выходе незачем: он уже прошёл валидацию при создании учётной записи,
    и повторная проверка способна только отказать там, где отказывать
    нечему. Так и случилось — адрес в домене `.local` (зарезервированном по
    RFC 6762) заставил `GET /api/auth/me` отвечать 500, и пользователь не мог
    войти вообще: фронтенд считает такой ответ признаком просроченного
    токена и удаляет его.

    Строго на входе, терпимо на выходе. Иначе любая запись, оказавшаяся в
    базе — через миграцию, импорт или смену правил валидации в новой версии
    библиотеки, — превращается в отказ обслуживания для её владельца.
    """
