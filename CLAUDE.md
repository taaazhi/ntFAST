...
**Запуск без обратного прокси.** uvicorn по умолчанию доверяет заголовку
`X-Forwarded-For` всем, кто пришёл с localhost, и подменяет адрес клиента до
того, как запрос попадёт в приложение. Ограничение частоты при этом обходится
подстановкой нового адреса в каждый запрос — проверено вживую. Запускать так:

```bash
FORWARDED_ALLOW_IPS="" uvicorn app.main:app --host 0.0.0.0 --port 8000
```

За прокси — `TRUST_PROXY_HEADERS=true`, `FORWARDED_ALLOW_IPS` с адресом
прокси и закрытый прямой доступ к порту. Приложение предупреждает в журнале,
если настройки расходятся.
